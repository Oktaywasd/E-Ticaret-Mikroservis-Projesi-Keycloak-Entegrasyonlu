...
class OrderClientsIntegrationTest {
    ...
    @Test
    @DisplayName("Sözleşme Başarılı: CRM ve ProductCatalog onay verdiğinde sipariş oluşturulmalı ve stok düşülmeli")
    void createOrder_WhenExternalServicesSucceed_ShouldCreateOrderAndReduceStock() {
        // WireMock 1: CRM Adres Getir
        ...
        // WireMock 2: ProductCatalog Ürün Getir (Stok: 10, Fiyat: 500.00)
        ...
        // WireMock 3: ProductCatalog Stok Düşme PUT çağrısı
    ...
    void createOrder_WhenInsufficientStock_ShouldThrowBusinessException() {
        // WireMock: CRM Adres OK
        stubFor(get(urlEqualTo("/api/v1/addresses/" + ADDRESS_ID))
                .willReturn(aResponse().withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody("{\"id\":\"" + ADDRESS_ID + "\"}")));

        // WireMock: ProductCatalog Ürün Stok: 1
        String productJsonResponse = """
                {
                    "id": "%s",
                    "name": "Koşu Ayakkabısı",
        ...
        CreateOrderRequestDto request = CreateOrderRequestDto.builder()
                .addressId(ADDRESS_ID)
                .items(List.of(new OrderItemRequestDto(PRODUCT_ID, 3))) // 3 adet isteniyor ama stok 1
                .build();
    ...
    @Test
    @DisplayName("Sözleşme İptal Akışı: Sipariş iptal edildiğinde restoreStock çağrılmalı")
    void cancelOrder_WhenCancelled_ShouldRestoreStockViaFeign() {
        stubFor(put(urlEqualTo("/api/v1/products/" + PRODUCT_ID + "/restore-stock?quantity=2"))
                .willReturn(aResponse().withStatus(200)));
        ...
        verify(putRequestedFor(urlEqualTo("/api/v1/products/" + PRODUCT_ID + "/restore-stock?quantity=2")));
    }
}